/********************************************************************************
** Form generated from reading UI file 'InputMethodNumber.ui'
**
** Created by: Qt User Interface Compiler version 5.12.8
**
** WARNING! All changes made in this file will be lost when recompiling UI file!
********************************************************************************/

#ifndef UI_INPUTMETHODNUMBER_H
#define UI_INPUTMETHODNUMBER_H

#include <QtCore/QVariant>
#include <QtWidgets/QApplication>
#include <QtWidgets/QGridLayout>
#include <QtWidgets/QPushButton>
#include <QtWidgets/QWidget>

QT_BEGIN_NAMESPACE

class Ui_InputMethodNumber
{
public:
    QGridLayout *gridLayout;
    QPushButton *btn7;
    QPushButton *btn8;
    QPushButton *btn9;
    QPushButton *btnClose;
    QPushButton *btn4;
    QPushButton *btn5;
    QPushButton *btn6;
    QPushButton *btnSpace;
    QPushButton *btn1;
    QPushButton *btn2;
    QPushButton *btn3;
    QPushButton *btnEnter;
    QPushButton *btn0;
    QPushButton *btn00;
    QPushButton *btnDot;
    QPushButton *btnDelete;

    void setupUi(QWidget *InputMethodNumber)
    {
        if (InputMethodNumber->objectName().isEmpty())
            InputMethodNumber->setObjectName(QString::fromUtf8("InputMethodNumber"));
        InputMethodNumber->resize(301, 239);
        gridLayout = new QGridLayout(InputMethodNumber);
        gridLayout->setObjectName(QString::fromUtf8("gridLayout"));
        btn7 = new QPushButton(InputMethodNumber);
        btn7->setObjectName(QString::fromUtf8("btn7"));
        QSizePolicy sizePolicy(QSizePolicy::Ignored, QSizePolicy::Expanding);
        sizePolicy.setHorizontalStretch(0);
        sizePolicy.setVerticalStretch(0);
        sizePolicy.setHeightForWidth(btn7->sizePolicy().hasHeightForWidth());
        btn7->setSizePolicy(sizePolicy);
        btn7->setFocusPolicy(Qt::NoFocus);
        btn7->setFlat(false);

        gridLayout->addWidget(btn7, 0, 0, 1, 1);

        btn8 = new QPushButton(InputMethodNumber);
        btn8->setObjectName(QString::fromUtf8("btn8"));
        sizePolicy.setHeightForWidth(btn8->sizePolicy().hasHeightForWidth());
        btn8->setSizePolicy(sizePolicy);
        btn8->setFocusPolicy(Qt::NoFocus);
        btn8->setFlat(false);

        gridLayout->addWidget(btn8, 0, 1, 1, 1);

        btn9 = new QPushButton(InputMethodNumber);
        btn9->setObjectName(QString::fromUtf8("btn9"));
        sizePolicy.setHeightForWidth(btn9->sizePolicy().hasHeightForWidth());
        btn9->setSizePolicy(sizePolicy);
        btn9->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btn9, 0, 2, 1, 1);

        btnClose = new QPushButton(InputMethodNumber);
        btnClose->setObjectName(QString::fromUtf8("btnClose"));
        sizePolicy.setHeightForWidth(btnClose->sizePolicy().hasHeightForWidth());
        btnClose->setSizePolicy(sizePolicy);
        btnClose->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btnClose, 0, 3, 1, 1);

        btn4 = new QPushButton(InputMethodNumber);
        btn4->setObjectName(QString::fromUtf8("btn4"));
        sizePolicy.setHeightForWidth(btn4->sizePolicy().hasHeightForWidth());
        btn4->setSizePolicy(sizePolicy);
        btn4->setFocusPolicy(Qt::NoFocus);
        btn4->setFlat(false);

        gridLayout->addWidget(btn4, 1, 0, 1, 1);

        btn5 = new QPushButton(InputMethodNumber);
        btn5->setObjectName(QString::fromUtf8("btn5"));
        sizePolicy.setHeightForWidth(btn5->sizePolicy().hasHeightForWidth());
        btn5->setSizePolicy(sizePolicy);
        btn5->setFocusPolicy(Qt::NoFocus);
        btn5->setFlat(false);

        gridLayout->addWidget(btn5, 1, 1, 1, 1);

        btn6 = new QPushButton(InputMethodNumber);
        btn6->setObjectName(QString::fromUtf8("btn6"));
        sizePolicy.setHeightForWidth(btn6->sizePolicy().hasHeightForWidth());
        btn6->setSizePolicy(sizePolicy);
        btn6->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btn6, 1, 2, 1, 1);

        btnSpace = new QPushButton(InputMethodNumber);
        btnSpace->setObjectName(QString::fromUtf8("btnSpace"));
        sizePolicy.setHeightForWidth(btnSpace->sizePolicy().hasHeightForWidth());
        btnSpace->setSizePolicy(sizePolicy);
        btnSpace->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btnSpace, 1, 3, 1, 1);

        btn1 = new QPushButton(InputMethodNumber);
        btn1->setObjectName(QString::fromUtf8("btn1"));
        sizePolicy.setHeightForWidth(btn1->sizePolicy().hasHeightForWidth());
        btn1->setSizePolicy(sizePolicy);
        btn1->setFocusPolicy(Qt::NoFocus);
        btn1->setFlat(false);

        gridLayout->addWidget(btn1, 2, 0, 1, 1);

        btn2 = new QPushButton(InputMethodNumber);
        btn2->setObjectName(QString::fromUtf8("btn2"));
        sizePolicy.setHeightForWidth(btn2->sizePolicy().hasHeightForWidth());
        btn2->setSizePolicy(sizePolicy);
        btn2->setFocusPolicy(Qt::NoFocus);
        btn2->setFlat(false);

        gridLayout->addWidget(btn2, 2, 1, 1, 1);

        btn3 = new QPushButton(InputMethodNumber);
        btn3->setObjectName(QString::fromUtf8("btn3"));
        sizePolicy.setHeightForWidth(btn3->sizePolicy().hasHeightForWidth());
        btn3->setSizePolicy(sizePolicy);
        btn3->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btn3, 2, 2, 1, 1);

        btnEnter = new QPushButton(InputMethodNumber);
        btnEnter->setObjectName(QString::fromUtf8("btnEnter"));
        sizePolicy.setHeightForWidth(btnEnter->sizePolicy().hasHeightForWidth());
        btnEnter->setSizePolicy(sizePolicy);
        btnEnter->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btnEnter, 2, 3, 1, 1);

        btn0 = new QPushButton(InputMethodNumber);
        btn0->setObjectName(QString::fromUtf8("btn0"));
        sizePolicy.setHeightForWidth(btn0->sizePolicy().hasHeightForWidth());
        btn0->setSizePolicy(sizePolicy);
        btn0->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btn0, 3, 0, 1, 1);

        btn00 = new QPushButton(InputMethodNumber);
        btn00->setObjectName(QString::fromUtf8("btn00"));
        sizePolicy.setHeightForWidth(btn00->sizePolicy().hasHeightForWidth());
        btn00->setSizePolicy(sizePolicy);
        btn00->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btn00, 3, 1, 1, 1);

        btnDot = new QPushButton(InputMethodNumber);
        btnDot->setObjectName(QString::fromUtf8("btnDot"));
        sizePolicy.setHeightForWidth(btnDot->sizePolicy().hasHeightForWidth());
        btnDot->setSizePolicy(sizePolicy);
        btnDot->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btnDot, 3, 2, 1, 1);

        btnDelete = new QPushButton(InputMethodNumber);
        btnDelete->setObjectName(QString::fromUtf8("btnDelete"));
        sizePolicy.setHeightForWidth(btnDelete->sizePolicy().hasHeightForWidth());
        btnDelete->setSizePolicy(sizePolicy);
        btnDelete->setFocusPolicy(Qt::NoFocus);

        gridLayout->addWidget(btnDelete, 3, 3, 1, 1);


        retranslateUi(InputMethodNumber);

        QMetaObject::connectSlotsByName(InputMethodNumber);
    } // setupUi

    void retranslateUi(QWidget *InputMethodNumber)
    {
        InputMethodNumber->setWindowTitle(QApplication::translate("InputMethodNumber", "\346\225\260\345\255\227\351\224\256\347\233\230", nullptr));
        btn7->setText(QApplication::translate("InputMethodNumber", "7", nullptr));
        btn8->setText(QApplication::translate("InputMethodNumber", "8", nullptr));
        btn9->setText(QApplication::translate("InputMethodNumber", "9", nullptr));
        btnClose->setText(QApplication::translate("InputMethodNumber", "\345\205\263\351\227\255", nullptr));
        btn4->setText(QApplication::translate("InputMethodNumber", "4", nullptr));
        btn5->setText(QApplication::translate("InputMethodNumber", "5", nullptr));
        btn6->setText(QApplication::translate("InputMethodNumber", "6", nullptr));
        btnSpace->setText(QApplication::translate("InputMethodNumber", "\347\251\272\346\240\274", nullptr));
        btn1->setText(QApplication::translate("InputMethodNumber", "1", nullptr));
        btn2->setText(QApplication::translate("InputMethodNumber", "2", nullptr));
        btn3->setText(QApplication::translate("InputMethodNumber", "3", nullptr));
        btnEnter->setText(QApplication::translate("InputMethodNumber", "\345\233\236\350\275\246", nullptr));
        btn0->setText(QApplication::translate("InputMethodNumber", "0", nullptr));
        btn00->setText(QApplication::translate("InputMethodNumber", "00", nullptr));
        btnDot->setText(QApplication::translate("InputMethodNumber", ".", nullptr));
        btnDelete->setText(QApplication::translate("InputMethodNumber", "\342\206\220", nullptr));
    } // retranslateUi

};

namespace Ui {
    class InputMethodNumber: public Ui_InputMethodNumber {};
} // namespace Ui

QT_END_NAMESPACE

#endif // UI_INPUTMETHODNUMBER_H
